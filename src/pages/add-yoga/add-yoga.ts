...
/**
 * Generated class for the AddYogaPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-add-yoga',
  templateUrl: 'add-yoga.html',
})
export class AddYogaPage {
  dateStart = '';
  dateEnd = '';
  timeStart = "00:00";
  timeEnd = "00:00";
  ...
  ionViewDidLoad() {
    ...
    console.log('ionViewDidLoad AddYogaPage');
  }
  save() {
    if (this.dateStart != '' && this.dateEnd != '' && this.get != 0) {
      ...
      this.count = count;
      if (this.count == 0) {
        firebase.firestore().collection("Yoga").doc('Yoga-1').set({
        ...
        if (t != 0) {
          console.log('t: ' + t);
          firebase.firestore().collection("Yoga").doc('Yoga-' + (t)).set({
            date: dateStart + '-' + dateEnd,
            time: timeStart + '-' + timeEnd,
            registered: 0,
            get: Number(get)
          ...
          firebase.firestore().collection("Yoga").doc('Yoga-' + (this.count + 1)).set({
            date: dateStart + '-' + dateEnd,
            time: timeStart + '-' + timeEnd,
            registered: 0,
            get: Number(get)
